...
export async function POST(req: NextRequest) {
  ...
  const { sessionId, content, settings } = body;
  if (!sessionId || !content) {
    return new Response(JSON.stringify({ error: "BAD_REQUEST", message: "sessionId and content are required" }), {
      status: 400,
      headers: { "Content-Type": "application/json" },
    });
  }
  ...
  // 4. Save User prompt into database message history
