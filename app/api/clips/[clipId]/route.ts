...
import type { ClipRow } from "@/features/clips/lib/schemas";
...
export async function GET(
  ...
  try {
    ...
    const { data: clips, error } = await insforge.database
      .from("video_clips")
      .select("*")
      .eq("id", clipId)
      .limit(1);
    ...
    const { data: members, error: memberError } = await insforge.database
      ...
      .eq("workspace_id", clip.workspace_id)
      .limit(1);
    ...
    if (!clip.r2_object_key) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: "Clip is not available yet",
      });
    }

    const { searchParams } = new URL(request.url);
    const objectKey = clip.r2_object_key;
